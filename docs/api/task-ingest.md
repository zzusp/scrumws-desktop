...
- **per-key 策略白名单**（创建密钥时可选配置，空 = 不限）：`allowedModels` / `allowedEfforts`（须为全局白名单子集）、`allowedCwds`（绝对路径列表；任务 `cwd` 须等于某项或在其之下，Windows 大小写不敏感）。请求省略对应字段时取白名单**首项**为该密钥默认；请求越界一律 `400 …不在该密钥允许范围…`。
