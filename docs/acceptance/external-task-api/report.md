...
**结论：matrix 全绿——round-1 30/30（唯一首跑 FAIL 是用例断言 bug，产品代码零返工）；round-2（per-key 策略 + 心跳增量）32/32 API + 5/5 UI + 派发器 e2e 全过，见 round-2.md。**
...
- **per-key 策略白名单**（round-2）：密钥可配 `allowedModels/allowedEfforts/allowedCwds`（空=不限；请求省略取首项为默认；越界 400；cwd 允许白名单目录及其子目录）。
