...
**结论：matrix 全绿——round-1 30/30；round-2（per-key 策略 + 心跳）32/32+5/5+派发器 e2e；round-3（策略必选）33/33+6/6；round-4（allowQueued 直执权限 + 编辑/复制）37/37+8/8；round-9（外部查询确认 + 原生续接）50/50。生产链路 e2e 见 D:\baibu-agent\docs\acceptance\scrumws-ingest-cutover\round-3.md。明细见各 round-N.md。**
...
| 外部任务模块（幂等台账 + 状态查询 + 原生续接 + source 隔离） | `platform/lib/external-ingest.js` |
...
- 鉴权失败（缺头/错钥/禁用/已删）统一 401，不泄露密钥状态；跨 source 查询或续接统一 404，不泄露他源任务存在性。
- 外部续接要求 `message`，只允许已收敛的 `awaiting-human` / `done` 任务；复用绑定 provider 的原生 resume，不能绕过 plan 的看板确认。
