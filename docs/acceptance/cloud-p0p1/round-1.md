...
> ⚠ **本段的 152ms 已被 round-3 证伪，勿据此做容量规划。** 本轮 bench 跑在 4 个任务包、
> **0 张 CLI 观察态卡**的沙箱上，外推模型漏了成本大头——CLI 卡要反读 CC 的 jsonl
> （`collect-cli.js:195`）。真实规模实测 **2.2ms/卡**（4.4×），300 张卡应为 **~660ms**。
> 见 [`round-3.md`](round-3.md)。以下原文保留以存档。
