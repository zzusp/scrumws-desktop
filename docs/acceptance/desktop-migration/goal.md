...
- **2026-07-11 R5**：孙鹏实际使用后验收反馈——**托盘菜单 OK**（C12 人工项闭环）；三条 UI 修订：去暗色只留亮色（themeBtn 保留元素但隐藏，防 app.js TypeError）、按钮/悬浮/选中配色严格按 multica 亮色范式、整体色系去 claude 橙对齐 multica（本轮**放开 app.js 颜色字面量**的修改权限，逻辑仍零改动）。**三条全部完成**（C17 PASS，见 round-5.md + 4 截图）：dark token 块删除、data-theme 硬编码 light、themeBtn 双类隐藏（首版单类被 .btn 反超没藏住，截图抓出修正）、悬浮/选中统一 --accent 范式、claude 橙及旧暗色 rgba 字面量清零（app.js 7 处颜色字面量、逻辑零改动）。
- **2026-07-11 R6**：孙鹏给参考截图 `docs/screenshots/landing-hero.avif`（multica Issues 看板实拍），要求看板按其风格样式颜色改。完成（C18 PASS，见 round-6.md）：状态列改 multica board 范式——列容器状态色浅底（plan/queued=muted 40%、processing=warning 5%、awaiting-human=destructive 5%、done=info 5%）+ 列头实心状态胶囊（lucide 图标白字）+ 计数白底小胶囊；双源基准（截图像素采样 × `packages/core/issues/config/status.ts`）互证，渲染像素对拍参考图全吻合（#DCA400 vs #DBA401 等）。仅 index.html 改动、app.js 零改动。**补充**：孙鹏指出菜单颜色没改——根因是 R5 侧边栏底(muted 0.967)与菜单选中胶囊(--accent 0.967)同色、选中态不可见；引入 multica sidebar 专用 token 组修复（底 --sidebar 0.985 + 选中 --sidebar-accent 0.95），像素回采 #FAFAFA/#EEEEF0 与参考图完全一致。
- **2026-07-11 R7**：孙鹏四项修订全部完成（C19 PASS，CDP 真实点击验证，见 round-7.md）——① 应用名改 **ScrumWS**（title/品牌/托盘/弹窗/productName，NSIS 安装包名随变）；② 顶栏 breadcrumb（ScrumWS › 页面名）；③ 看板来源筛选行（全部/dws/issue/manual/CLI；筛选态挂 #view-board 属性 + 卡片 data-source + CSS 匹配，周期重渲染不冲掉）；④ 新建任务按钮去黑底改白底描边；⑤ **归档拆独立菜单**（#/archive 路由 + 独立 view 默认展开，app.js 仅 3 处小改：ROUTE_VIEWS/route 解析/卡片 data-source 属性）。
