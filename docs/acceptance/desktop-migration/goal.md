...
- **2026-07-11 R6**：孙鹏给参考截图 `docs/screenshots/landing-hero.avif`（multica Issues 看板实拍），要求看板按其风格样式颜色改。完成（C18 PASS，见 round-6.md）：状态列改 multica board 范式——列容器状态色浅底（plan/queued=muted 40%、processing=warning 5%、awaiting-human=destructive 5%、done=info 5%）+ 列头实心状态胶囊（lucide 图标白字）+ 计数白底小胶囊；双源基准（截图像素采样 × `packages/core/issues/config/status.ts`）互证，渲染像素对拍参考图全吻合（#DCA400 vs #DBA401 等）。仅 index.html 改动、app.js 零改动。
