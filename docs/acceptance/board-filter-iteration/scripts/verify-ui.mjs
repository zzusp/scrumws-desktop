...
try {
  ...
  const srcChips = await page.evaluate(() => [...document.querySelectorAll('#fpSourceChips .fp-chip')].map((c) => c.textContent.trim()));
  // 来源按字母序：chat→dws / issue→issue / manual→Manual（数据里没有 cli 任务 → 不出 CLI 选项，证明取自真实数据）
  ok(JSON.stringify(srcChips) === JSON.stringify(['全部', 'dws', 'issue', 'Manual']),
    `req1：来源选项取自真实数据（${srcChips.join('/')}）`);

  // 点 Manual chip → 只剩 2 张 manual 卡（alpha plan + delta done）
  await page.evaluate(() => [...document.querySelectorAll('#fpSourceChips .fp-chip')].find((c) => c.textContent.trim() === 'Manual').click());
  ...
  // ---- req3：工作目录筛选，选项取真实 cwd ----
  const cwdOpts = await page.evaluate(() => [...document.querySelectorAll('#fpCwd option')].map((o) => o.value));
  ...
  // 先清来源，改选 cwd=另一 repo → chat bravo + manual delta
  await page.evaluate(() => [...document.querySelectorAll('#fpSourceChips .fp-chip')].find((c) => c.textContent.trim() === '全部').click());
  await page.select('#fpCwd', 'D:\\work\\another-repo\\packages\\ui');
