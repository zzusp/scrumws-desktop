...
**部署 P2 / 手机端个人控制台时对应的 compose 改动**：

```yaml
environment:
  - CLOUD_INSECURE_COOKIE=1
  - CLOUD_ACCEPT_PLAINTEXT_DISPATCH=1   # 下行面（P2 派活 + 手机中继）必须显式加；不加则下行 503，只读面照常
```

> **手机端个人控制台已上线**（`docs/spec/cloud-mobile-console.md`，`/m/` 同源托管）——它是这个下行面的**第一个消费者**：
> 手机页的查看/新建/回复/实时输出经云端中继到机器主人**本机** app（相当于对本机的远程执行面）。
> 因此同一把 `CLOUD_ACCEPT_PLAINTEXT_DISPATCH` 现在也门控它：**不设 → 手机页登录能开、但一切 `/api/my/*` 与 downlink 返回 503**，控制台不可用。
> 另有两道**本地**闸门不依赖云端诚实（机器主人在桌面设置页控）：`cloudRemoteControl` 总开关（默认关）+ `cloudAllowedCwds` 新建白名单（默认空）。绊线的完整语义与安全论证见设计 §7 与 `cloud-mobile-console.md §2/§3`。
