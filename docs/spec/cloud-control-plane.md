...
> ⚠ **2026-07-17 起，本文的「任务」模型已被 [`cloud-pipeline-model.md`](cloud-pipeline-model.md) 增补/部分推翻**：
> 引入 Issue / Stage / Run 三层以支持「一个需求在多个 agent 之间轮转」，且**本地任务不再全量上云**
> （只同步云端下发的）。本文的 §5 数据模型、§6.3 对账、§9 决策 5 受影响。
> P0+P1 的已交付部分（机器注册 / 心跳 / 对账 / 部署）不受影响，仍以本文为准。
...
| ~~**强制 HTTPS**~~ | **已被决策 13 推翻**：`CLOUD_INSECURE_COOKIE=1` 按内网模式跑裸 HTTP。风险与 P2 绊线见 §7.5 |
...
### 7.5 决策 13 的风险接受与 P2 绊线 ⚠

**决策**：按内网模式部署（`CLOUD_INSECURE_COOKIE=1`，裸 HTTP、不前置 TLS）。**用户已明确知悉实际会部署到公网，并接受该风险。** 记在这里不是为了甩锅，是因为这个判断**有前提、且前提会过期**。

**风险随阶段跃迁——这是本条的关键**：

| 阶段 | `swuk_` / `swmt_` 明文泄露后 | 判断 |
|---|---|---|
| **P0+P1（当前）** | 看到全团队任务看板 / 往云端推假状态。**无下行通道 = 无 RCE 面** | 风险有界，可接受 |
...
原设计：`CLOUD_INSECURE_COOKIE=1` 且非 localhost → **拒绝启动**。

**用户明确要求改判据**（不接受被拒绝启动，明知服务在公网裸 HTTP 上仍要 P2 能跑）。修订为**显式确认**而非拒绝：

> P2 意图下行上线时，`CLOUD_INSECURE_COOKIE=1` + 非 localhost **不再阻止启动**，但必须另设 `CLOUD_ACCEPT_PLAINTEXT_DISPATCH=1` 才放行下行；缺它则**只启动 P0+P1 的只读面**，下行端点返回 503。启动日志必须打出当前姿态。

**为什么改判据而不是整个删掉**：绊线的用途从来不是"拦住用户"，是**防止一个有前提的判断被静默继承**。决策 13 是在「P1 只读、风险有界」下做的；P2 让前提失效。删掉绊线 = 那个前提永远消失，几周后没人记得公网裸 HTTP 曾经只是因为"反正只读"才被接受。

改成第二把显式开关，两个性质都保住了：

- 用户要的——**P2 能在这个部署上跑**，不被拒绝启动
- 该保的——**风险不会被静默继承**。开 P2 下行的人必须亲手写下 `ACCEPT_PLAINTEXT_DISPATCH`，那一刻他就知道自己在接受什么，而不是无意中继承了几周前为「只读」做的决定

**要 P2 下行 = 明写"我接受明文派活"**。这句话本身就是那道闸。

**为什么不用两端对称加密代替 TLS**（评估过，否决）：

- **浏览器那条是死结**。页面 JS 经 HTTP 送达，路径上的人可直接**改写这段 JS**，让它把 `swuk_` 发到别处。在被篡改的代码里做任何加密都是自欺。而 `swuk_` 恰是权限最大的凭据（P2 后能给所有机器派活）——对称加密救不了最严重的那个暴露面。
- **connector↔云端那条技术上可行**（两端都是我们的代码，可预共享密钥），但要自己补齐 TLS 白送的：防重放（对称加密不防，抓包原样重放即可）、密钥分发（走 enroll 就是明文过 HTTP，全盘皆输，只能带外配）、前向保密（没有，密钥一泄历史全解）。外加 IV 重用 / 非 AEAD / 时序比较这些自研 crypto 的常见翻车点。
- **成本对比悬殊**：Caddy 自动签证书约五行配置，白送机密性+完整性+身份+防重放+前向保密，且覆盖浏览器链路；自研对称加密工作量大得多，**只保护一半**。

结论：要么内网裸 HTTP（链路可信前提成立），要么 TLS 反代。对称加密处在"比裸奔强、比 TLS 差很多、还得自己维护"的位置，不取。

### 7.6 既存问题（本设计不引入，但会放大）
...
1. ~~**`collect.js` 加缓存层的影响面**（P1 唯一的技术不确定点）~~ → **已收敛**，见 [`../acceptance/cloud-p0p1/round-3.md`](../acceptance/cloud-p0p1/round-3.md)。

   真实数据根实测（51 张卡，含 25 张 CLI 观察态卡，jsonl 1683 文件/564MB）：`collectState()` **P50 = 112.2ms**，是 4 包沙箱（4.9ms）的 **23 倍**；摊薄 **2.2ms/卡**。**round-1 的「~0.5ms/包、300 包=152ms」外推低估 4.4 倍，已证伪并就地标注**——它漏了 CLI 卡反读 jsonl 这块大头，因为沙箱里一张 CLI 卡都没有。

   **模型错了，但设计对了，且论据更硬**：single-flight 从「省 93ms 的锦上添花」变成「省 **2.2 秒**连续扫描的承重墙」（20 并发实测只扫 1 次）；「connector 不得触发额外扫描」从省 4.9ms/15s 变成省 **112ms/15s**（实测 +0）；写代次在真实规模下同样成立。**TTL 从不降级 UI 新鲜度**不变（UI 轮询 5s > TTL 3s，每次必然重扫）。

   容量规划按 **~2.2ms/卡**，且成本主要由 CLI 卡驱动。
