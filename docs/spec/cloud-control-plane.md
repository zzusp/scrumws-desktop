...
**P2 绊线（必须实现）**：P2 的意图下行代码上线时，若 `CLOUD_INSECURE_COOKIE=1` 且服务并非绑在 localhost → **拒绝启动**，不是打警告了事。

理由：靠人记得"当初设过一个 env、P2 前要回来重新评估"是不可靠的——这类跨越数周、跨越 PR 的口头约定就是会被忘掉。把**会过期的判断做成会失效的代码**，是唯一可靠的兑现方式。这不是保守，是让决策在变危险的那一刻自动兜底。
