...
全团队 agent 运行态的**派活面**：机器在线状态 + **云端下发任务**的执行镜像。

- **P0 身份与连线**：登录密钥 / 注册密钥 / 配对码 / enroll / 心跳 / 机器列表
- **P1 状态上行**：机器增量上报任务卡 + 每 5min 全量对账 → 云端看板 / 任务详情。
  **决策 14（已落地）**：只上行「云端下发」的任务（对账按 cloud link 过滤，`platform/lib/cloud/links.js` + `reconcile.js`）；本地手敲 / CLI / API / 手机建的活**不上云**——云端是派活平台不是监控大盘，主人看自己机器全部任务走 `/m/` 实时中继。P2 下行未落地前 links 为空 = 云端任务看板暂空（决策 14 的目标终态）。
